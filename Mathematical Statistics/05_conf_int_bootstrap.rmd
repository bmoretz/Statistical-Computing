...
for(i in 1:N)
{
   samp.target <- sample(prices.target, size = length(prices.target), replace = T)
   samp.walmart <- sample(prices.walmart, size = length(prices.walmart), replace = T)
...
The bootstrap confidence intervals include zero, so there is not sufficent evidence here to support the claim that the prices have a statistically significant difference.