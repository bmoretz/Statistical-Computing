...
res <- sapply(years, FUN = function(year){
   
   print(paste("loading year...", year))
   
   file <- read_results("men", year)
   
   parsed <- extractVariables(file)
})

file <- read_results("men", 2006)
m2006 <- extractVariables(file)
...
mfilenames <- file.path(data.dir, paste("men_txt/", 1999:2012, ".txt", sep = ""))
